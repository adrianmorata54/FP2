...
# ==============================================================================
# KNN PONDERADO REAL (EJERCICIO 4)
# ==============================================================================

class Regresor_kNN_Ponderado(Regresor_kNN):
    """
    Regresor kNN usando distancia euclídea ponderada.
    Los pesos indican la importancia de cada lag.
    """

    def __init__(self, k: int, pesos: list[float]):
        super().__init__(
            k=k,
            distancia="ponderada",
            pesos=pesos
...
# ==============================================================================
# FUNCIÓN OBJETIVO PARA HEURÍSTICAS (EJERCICIO 5)
# ==============================================================================

class FuncionObjetivoKNN:
    """
    Función objetivo para optimizar los pesos del kNN ponderado.
    
    El optimizador intentará MINIMIZAR el MAE.
    """
    ...
    def __init__(self, dataset, k):
        ...
        # Número de pesos necesarios = número de atributos (lags)
        self.dimension = len(dataset.registros[0].atributos)

    def evaluar(self, pesos):
        ...
        if len(pesos) != self.dimension:
            raise ValueError(
                f"Se esperaban {self.dimension} pesos y llegaron {len(pesos)}"
            )
        ...
        for reg in self.dataset.registros[:100]:

            pred = modelo.predecir(reg)

            error = abs(pred - reg.objetivo)
...
# ==============================================================================
# KNN CON CORRELACIÓN DE PEARSON
# ==============================================================================

class Regresor_kNN_Correlacion(Regresor_kNN):
    """
    Variante de kNN que usa correlación de Pearson
    ...
    def predecir(self, registro):
        ...
        for vecino in self.datos_entrenamiento.registros:

            # Evitar compararse consigo mismo
            if vecino is registro:
                continue

            sim = registro.correlacion_pearson(vecino)

            similitudes.append((sim, vecino))
        ...
        vecinos = similitudes[:self.k]

        promedio = sum(
            vecino.objetivo
            for _, vecino in vecinos
        ) / self.k
