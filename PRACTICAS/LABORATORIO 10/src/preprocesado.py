...
class NormalizadorMaxMin(Preprocesamiento):
    ...
    def destransformar_valor(self, valor: float, indice: int = 0) -> float:
        """
        Convierte un valor normalizado [0,1] de vuelta a su escala original.
        """
        return valor * (self.maximos[indice] - self.minimos[indice]) + self.minimos[indice]
...
class NormalizadorZ_Score(Preprocesamiento):
    ...
    def destransformar_valor(self, valor: float, indice: int = 0) -> float:
        """
        Convierte un valor estandarizado Z-Score a la escala original.
        """
        return valor * self.desviaciones[indice] + self.medias[indice]
