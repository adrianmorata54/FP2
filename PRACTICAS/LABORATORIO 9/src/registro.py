...
class Registro:
    ...
    def normalizar(self, minimos: list[float], maximos: list[float]):
    ...
    def estandarizar(self, medias: list[float], desviaciones: list[float]):
    ...
    def correlacion_pearson(self, otro: 'Registro') -> float:
        """
        (LABORATORIO 9)
        Calcula el grado de relación lineal entre dos vectores.
        Rango: [-1, 1]. Siendo 1 una similitud total.
        """
        if len(self.atributos) != len(otro.atributos):
            raise ValueError("Los registros deben tener la misma dimensión.")
            
        n = len(self.atributos)
        media_x = sum(self.atributos) / n
        media_y = sum(otro.atributos) / n
        
        # Numerador: Covarianza
        numerador = sum((x - media_x) * (y - media_y) for x, y in zip(self.atributos, otro.atributos))
        
        # Denominador: Producto de desviaciones típicas
        sum_cuad_x = sum((x - media_x)**2 for x in self.atributos)
        sum_cuad_y = sum((y - media_y)**2 for y in otro.atributos)
        
        denominador = math.sqrt(sum_cuad_x * sum_cuad_y)
        
        if denominador == 0:
            return 0.0 # Para evitar errores si una serie es plana
